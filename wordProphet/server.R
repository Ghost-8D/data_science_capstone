...
# Author: Panayiotis L.
# Date: 03 Oct 2020
# Description: Coursera Data Science Capstone Project (shiny server)
...
predictionMatch <- function(userInput, ngrams) {
    ...
    if (ngrams == 3) {
        userInput1 <-
            paste(userInput[length(userInput) - 1], userInput[length(userInput)])
...
shinyServer(function(input, output) {
    ...
    observe({
        ...
        if (numPredictions == 1) {
            output$prediction1 <-
                reactive({
                    predictNextWord(input$userInput, 1)
                })
            ...
            output$prediction2 <-
                reactive({
                    predictNextWord(input$userInput, 2)
                })
            output$prediction3 <- NULL
            ...
            output$prediction3 <-
                reactive({
                    predictNextWord(input$userInput, 3)
                })
        }
    })
