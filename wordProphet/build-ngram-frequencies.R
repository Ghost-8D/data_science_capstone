...
setwd("~/Desktop/data_science_capstone/wordProphet")
...
if (!file.exists("data/final/en_US")) {
    tempFile <- tempfile()
    download.file(trainURL, tempFile, method="curl")
...
# blogs
blogsFileName <- path_to_blogs
con <- file(blogsFileName, open = "r")
blogs <- readLines(con, encoding = "UTF-8", skipNul = TRUE)
close(con)

# news
newsFileName <- path_to_news
con <- file(newsFileName, open = "r")
news <- readLines(con, encoding = "UTF-8", skipNul = TRUE)
close(con)

# twitter
twitterFileName <- path_to_twitter
con <- file(twitterFileName, open = "r")
twitter <- readLines(con, encoding = "UTF-8", skipNul = TRUE)
close(con)

print("Loaded training data")
print(paste0("Number of lines per file (blogs):     ", format(length(blogs), 
                                                              big.mark = ",")))
print(paste0("Number of lines per file (news):    ", format(length(news), 
                                                            big.mark = ",")))
print(paste0("Number of lines per file (twitter): ", format(length(twitter), 
                                                            big.mark = ",")))
...
# load bad words file
badWordsFile <- "data/full-list-of-bad-words.txt"
...
sampleData <- gsub("[0-9](?:st|nd|rd|th)", "", sampleData, ignore.case = FALSE, 
                   perl = TRUE)
...
# remove punctuation
sampleData <- gsub("[^\\p{L}'\\s]+", "", sampleData, ignore.case = FALSE, 
                   perl = TRUE)
...
tokenFrequency <- function(corpus, n = 1, rem_stopw = NULL) {
    corpus <- tokens(corpus) %>% tokens_ngrams(n = n) %>% dfm()
    ...
    if (n > 1) {
        ...
        corpus$token <- word(string = corpus$token, start = 1, end = n - 1, 
                             sep = fixed(" "))
    }
    setorder(corpus, -n)
